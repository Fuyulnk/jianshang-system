...
import { computeFormulaValueMap, formatLedgerDisplayText } from '../../utils/financeLedgerFormula'
import { buildMergeOwnerMap, defaultLedgerCellStyle, parseLedgerCellStyle, resolveLedgerCellStyle, resolveLedgerCellTarget } from '../../utils/financeLedgerGrid'
...
const cellStyleMap = computed(() => {
  const map = new Map()
  for (const cell of cells.value) map.set(`${cell.row_index}:${cell.col_index}`, parseLedgerCellStyle(cell.style_json))
  return map
})
const formulaValueMap = computed(() => computeFormulaValueMap(cells.value))
...
const mergeOwnerMap = computed(() => buildMergeOwnerMap(merges.value))
...
const activeCellStyle = computed(() => {
  if (!selectedSingleCell.value) return defaultLedgerCellStyle
...
function resolveCellTarget(row, col) {
  return resolveLedgerCellTarget(row, col, mergeOwnerMap.value)
}
...
async function applySelectedCellStyle(partial) {
  ...
  const nextStyle = parseLedgerCellStyle({ ...current, ...partial })
